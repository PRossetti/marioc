...
void* planificador()
{
	...
	while(1)
	{
		...
		if(nuevosValores->hayNuevosValores == 1){
			...
			nuevosValores->hayNuevosValores = 0;
			log_debug(logPlanificador, "Entre al IF cabeza de Seba");
				...
				switch(msj->tipoMensaje)
				{
					...
					// cuidado aca porque podria ser una desconexion de nivel me parece ...
						...
						if ( descriptor != socket_Nivel) {
							...
							recursosALiberar = ""; // para no enviar un null
							...
							for ( i = 0; i < tamanio_cola(personaje->recursos); i++ ) {
								...
								if ( j < tamanio_cola(elem->colaBloqueados) ) {//significa q alguien se desbloquea con ese recu
									pjBloq->estado = LISTO;
									pjBloq->recursoEnEspera = "";
							...
							enviarMensaje(socket_Nivel,e_matar_interbloq,recursosALiberar,logPlanificador);
					...
					/*case e_finalizoNivel: //CAMBIAR POR EL Q CORRESPONDE
						...
						//busco el personaje segun el socket
						for (i = 0; i < (tamanio_cola(elem->colaListos)); ++i)
						{
						  personaje=obtener_contenido_pos_determinada(elem->colaListos, i);
						  if(personaje->fd==descriptor)
						...
						desecharSocket(descriptor,servidorPlanificador);
						//SRDF
						if( (strcmp(algoritmoQuantumRetardo[0], "SRDF") == 0) && (personaje == pers_actual) )
						{
							pers_actual = NULL;
						}
						break;
					*/
					case e_matar_interbloq:
						...
						for ( i = 0; i < tamanio_cola(personaje->recursos); i++ ) {
							recursoBloqueante = obtener_contenido_pos_determinada(personaje->recursos,i);
							log_debug(logPlanificador,"Tamanio cola: %d Elemento %d %s",
									tamanio_cola(personaje->recursos),i,recursoBloqueante);
						}
						*/
		...
		if (tamanio_cola(elem->colaListos)>0) // if para no elija un personaje que no existe
		{
			...
			while (q)
			{
				...
				usleep(useg);
				log_debug(logPlanificador,"Entre al while q y pers_actual->df: %d",pers_actual->fd);// le agregue esto para chequear algo PABLO
				...
				switch(msj->tipoMensaje)
				{
					...
					*/
					...
					case e_desconexion:
						...
						descriptor = pers_actual->fd; //PABLO
					...
					case e_recurso:
						q--;
						pers_actual->recursoEnEspera = msj->mensaje; //PABLO
						log_debug(logPlanificador,"CLARIGORDA El recurso en espera es %c",msj->mensaje);
						...
						TprocesoPersonaje *auxProcesoPersonaje;
							...
							if (control == e_error) {
								...
								pushear_cola(pers_actual->recursos,pers_actual->recursoEnEspera); // PABLO
...
int desbloquearPersonajes(int socket_Nivel,TprocesoPersonaje* personaje,tipoNiveles* elem,int tipo,t_log* logPlanificador) {
	...
	for ( i = 0; i <= tamanio_cola(personaje->recursos); i++ ) {
		i = 0;
		recursoBloqueante = obtener_contenido_pos_determinada(personaje->recursos,i);
		remover_cola_pos_determinada(personaje->recursos,i); // lucas
		...
		if ( j < tamanio_cola(elem->colaBloqueados) ) {//significa que alguien se desbloquea con ese recu
			...
			pushear_cola(pjBloq->recursos,recursoBloqueante);// Pablo
			// avisar al personaje que siga con el proximo recurso en su plan
			pushear_cola(elem->colaListos,pjBloq);
			...
			remover_cola_pos_determinada(elem->colaBloqueados,j);
				...
				log_debug(logPlanificador,"LUCAS TIENE RAZON");
			}
