...
int main(int argc, char* argv[]) {
	...
	if(cantNiveles == 0 )
	{
		...
		/*char* miIp = config_get_string_value(Personaje, "miIp"); //GLOBAL
		int miPuerto = config_get_int_value(Personaje, "miPuerto"); //EL PUERTO CAMBIA PARA CADA HILO ?
		char* orq=config_get_string_value(Personaje, "orquestador"); //GLOBAL
		char* ipOrq=strtok(orq, ":"); //GLOBAL
		int portOrq=atoi(strtok(NULL, ":")); //GLOBAL
		s_orq_main = crearCliente(miIp, miPuerto, logger);
		conectarCliente(s_orq_main, ipOrq, portOrq, logger);//me conecto con el Orquestador
		enviarMsjConOSinSenial(s_orq_main->sock->idSocket,e_finalizoPlanNiveles,"",1, logger);
		*/
		// BEGIN PABLO
...
void* procPersonaje( void* ptr ) {
	...
	//presentacion=concat(3, config_get_string_value(Personaje, "nombre"), ":", simbolo);
